...
def get_strikes_or_ball(user_input_number, random_number):
    ...
    for idx in range(3):
        if user_input_number[idx] == random_number[idx]:
            strikes += 1
        elif user_input_number[idx] in random_number:
            ball += 1
    result = [strikes,ball]
    # ==================================
    return result
...
def is_yes(one_more_input):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    one_more_input_low = one_more_input.lower()
    result = one_more_input_low == 'y' or one_more_input_low == 'yes'
    # ==================================
    return result
...
def is_no(one_more_input):
    ...
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    one_more_input_low = one_more_input.lower()
    result = one_more_input_low == 'n' or one_more_input_low == 'no'
    # ==================================
    return result
...
def main():
    ...
    while play_game:
        ...
        if strikes == 3:
            ...
            while not is_no(y_n) and not is_yes(y_n):
                print('Wrong Input, Input again')
                y_n = input('You win, one more(Y/N) ?')
            if is_no(y_n):
                play_game = False
